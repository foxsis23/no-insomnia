  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white/5 rounded-2xl border border-white/10 p-8">
        <h2 className="text-xl font-bold text-white mb-6">Повний розбір вашого типу</h2>
        <div className="prose prose-slate prose-sm max-w-none">
          {result.fullText.split('\n\n').map((block, i) => {
            if (block.startsWith('## ')) {
              return <h3 key={i} className="text-base font-bold text-white mt-6 mb-2">{block.replace('## ', '')}</h3>
            }
            ...
            return <p key={i} className="text-slate-300 text-sm leading-relaxed mb-3">{block}</p>
          })}
        </div>
      </div>
      ...
      {upsellProducts.length > 0 && (
        <div>
          <h3 className="font-semibold text-white mb-4">Наступний крок для вашого типу</h3>
          <div className="grid sm:grid-cols-3 gap-4">
            {upsellProducts.map((product) => (
              <div key={product.id} className="bg-white/5 border border-white/10 rounded-xl p-5 flex flex-col">
                ...
                <p className="text-slate-400 text-xs leading-relaxed flex-1 mb-3">{product.description}</p>
                <div className="flex items-center justify-between">
                  <span className="font-bold text-white">{product.price} грн</span>
                  <Link
                    href={product.id === 'course' ? '/course' : `/offer?product=${product.id}`}
                    className="text-indigo-400 hover:text-indigo-400 text-xs font-medium"
                  >
                    Придбати →
                  </Link>
                </div>
